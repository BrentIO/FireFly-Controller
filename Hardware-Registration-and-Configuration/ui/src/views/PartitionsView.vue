<template>
  <AppLayout>
    <div>
      <h2 class="text-xl font-semibold text-gray-900 mb-6">Partition Table</h2>

      <div v-if="loading" class="text-gray-500 text-sm">Loading…</div>

      <div
        v-else-if="errorMsg"
        class="bg-red-50 border border-red-200 text-red-800 text-sm rounded-md px-4 py-3 mb-4"
      >
        {{ errorMsg }}
      </div>
...
function formatSubtype(type, subtype) {
  let map = type === 0 ? APP_SUBTYPES : type === 1 ? DATA_SUBTYPES : {}
...
async function load() {
  ...
  try {
    ...
    const hasErrors = rows.value.some(r => r.errors.length > 0)
    setNavError('partitions', hasErrors)
  } catch (_) {
